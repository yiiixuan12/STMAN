"""
Single-seed training script for DMFGCRN.
Adapted from run.py to run only 1 seed for faster baseline evaluation.
"""
import os
import sys
...
# Also ensure the DMFGCRN directory is in path
dmfgcrn_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, dmfgcrn_dir)
...
from model.DMFGCRN import DMFGCRN as Network
...
args.add_argument('--model', default='DMFGCRN', type=str)
args.add_argument('--cuda', default=True, type=bool)
args.add_argument('--seed', default=12, type=int)
args1, _ = args.parse_known_args()

# get configuration
config_file = os.path.join(dmfgcrn_dir, 'config_file', '{}_{}.conf'.format(args1.dataset, args1.model))
...
if args.mode == 'train':
    seed = args.seed
    setup_seed(seed)
    log_dir = os.path.join(current_dir, 'experiments', args.dataset, current_time, str(seed))
