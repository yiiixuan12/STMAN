"""
Single-seed training script for MTEGCRN.
Adapted from DMFGCRN run_single.py.
"""
import os
import sys
...
mtegcrn_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, mtegcrn_dir)
...
from model.MTEGCRN import MTEGCRN as Network
...
_torch_threads = int(os.environ.get('BASELINE_CPU_THREADS', os.environ.get('OMP_NUM_THREADS', '4')))
...
args.add_argument('--model', default='MTEGCRN', type=str)
args.add_argument('--cuda', default=True, type=bool)
args.add_argument('--seed', default=12, type=int)
args1, _ = args.parse_known_args()

# get configuration
config_file = os.path.join(mtegcrn_dir, 'config_file',
                           '{}_{}.conf'.format(args1.dataset, args1.model))
...
train_loader, val_loader, test_loader, scaler = get_dataloader(
    args, normalizer=args.normalizer, tod=args.tod, dow=False,
...
if args.mode == 'train':
    ...
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=args.lr_init, eps=1.0e-8,
    ...
    trainer = Trainer(model, loss, optimizer, train_loader, val_loader,
                      test_loader, scaler, args, lr_scheduler=lr_scheduler)
