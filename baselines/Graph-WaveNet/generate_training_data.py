...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_dir):
        existing_files = [name for name in os.listdir(args.output_dir) if name.endswith(".npz")]
        if existing_files and not args.overwrite:
            raise FileExistsError(f"{args.output_dir} already has npz files; pass --overwrite to replace them")
    else:
