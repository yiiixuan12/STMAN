...
# 小工具
# ----------------------------
def _amp_enabled_for(device: torch.device) -> bool:
    disabled = os.environ.get("BASELINE_DISABLE_AMP", "0").strip().lower()
    return device.type == "cuda" and disabled not in {"1", "true", "yes", "on"}
...
def load_checkpoint(path: str,
    ...
    def _materialize_lazy_modules():
        ...
        try:
            ...
            x = torch.zeros(1, seq_len, n_nodes, in_dim, device=device)
            y = torch.zeros(1, pred_len, n_nodes, out_dim, device=device)
            with torch.no_grad():
                _forward_bridge(model, x, y, model_kind, teacher_forcing_ratio=0.0)
        except Exception:
            return
    ...
    try:
        ...
        message = str(exc)
        if "Unexpected key(s)" in message and ckpt.get("config", {}).get("model_kind") == "lstnn":
            _materialize_lazy_modules()
            try:
                model.load_state_dict(ckpt["model"], strict=True)
                loaded = True
                print("[Resume] Materialized lazy LSTNN modules before strict checkpoint load.", flush=True)
            except RuntimeError:
                raise exc
        if (not loaded) and "Unexpected key(s)" in message and "Missing key(s)" not in message:
            incompatible = model.load_state_dict(ckpt["model"], strict=False)
            print(
                "[Resume] Loaded checkpoint with extra unused keys ignored: "
...
def _forward_bridge(
    ...
    if model_kind == "lstnn":
        y_hat = model(x, future_data=y)      # [B, H, N]
...
def train_one_epoch(
    ...
    amp_enabled = _amp_enabled_for(device)
    ...
    for x, y in loader:
        ...
        autocast_ctx = torch.cuda.amp.autocast(dtype=torch.float16) if amp_enabled else nullcontext()
        with autocast_ctx:
...
def train_model(
    ...
    if resume_ckpt is not None and os.path.isfile(resume_ckpt):
        ...
        # Some baselines lazily instantiate submodules during the first forward.
        # Capture the resumed best state after the first train/eval pass so the
        # state_dict has the complete key set.
        best_state = None
    ...
    # 打印 AMP 状态
    if _amp_enabled_for(device):
        print("[AMP] Enabled: autocast(fp16) + GradScaler")
    elif device.type == "cuda":
        print("[AMP] Disabled by BASELINE_DISABLE_AMP")
    else:
